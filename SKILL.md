...
**选股(走矩阵 10 漏斗筛选):**
- 用户让选股/推荐股票/找能买的("帮我选几只"/"有什么能买的"/"哪个行业值得看"/"推荐几只成长股")
- 用户问"半导体板块还能看吗"/"现在什么方向值得买"等方向性问题
...
digraph flow {
    "用户输入" -> "判断:单股票 or 选股?";
    "判断:单股票 or 选股?" -> "单股票:走 analyze.py";
    "判断:单股票 or 选股?" -> "选股:走矩阵 10 漏斗";
    "单股票:走 analyze.py" -> "政治/地缘风险核查";
    "政治/地缘风险核查" -> "按框架解读";
    "按框架解读" -> "输出 9 段报告";
    "选股:走矩阵 10 漏斗" -> "质地筛 -> 位置筛 -> 资金流筛 -> 五步筛";
    "质地筛 -> 位置筛 -> 资金流筛 -> 五步筛" -> "候选 3-5 只逐只 analyze.py";
    "候选 3-5 只逐只 analyze.py" -> "10 维度打分排名";
    "10 维度打分排名" -> "输出推荐 1-2 只 + 理由 + 缺哪些维度";
}
```
...
## 矩阵 10:选股方法论(漏斗筛选理想买点)

**核心思想:** 大部分股票大部分时间都不在理想买点。选股 = 漏斗筛选,而非"找机会"。**等机会,不是找机会。**

### 10.1 理想买点画像(10 维度同时满足)

把五步核对 + 矩阵 1/1.5/2/3/8/9 反着读,理想买点必须同时满足:

| 维度 | 理想值 | 含义 | 数据源 |
|------|--------|------|--------|
| 1. 位置 | 120日分位 < 30% | 低位,非中高位 | `position.pct_120d` |
| 2. 量价 | 量增价涨 或 量缩价涨+均线陡峭 | 主力扫货或高控盘 | `trend_5d.quadrant` + `volume_price_detail` |
| 3. 换手率 | 今日 5-10%(活跃) + 5日均 > 20日均 | 主力参与,非套现 | `turnover` |
| 4. 量比 ROVL | 1.5-3.0 | 主力放量进场,非自然缩量也非一日游 | `five_step.3_vol_ratio` |
| 5. 均量线 | 5日金叉20日 | 趋势确认 | `five_step.4_vol_ma_cross` |
| 6. 突破站稳 | 连续 3 日 > 20日最高价 | 真突破 | `five_step.5_breakout_3day` |
| 7. 筹码 | 低位单峰 + 集中度 ≥40% + 底仓不动 | 主力吸筹完成 | `chip.pattern` + `concentration_5pct` + `bottom_retention` |
| 8. 资金流 | 持续流入 + ma5 > ma20 金叉 + 连续 ≥2 日流入 | 真金白银进场 | `capital_flow` |
| 9. 公司质地 | 成长股(ROIC 稳定+利润正增长+FCF 良好)或周期股低谷(PE 极高/亏损) | 不踩周期顶部 | `fundamental.classification` |
| 10. 新闻 | 偏正面 + 关键事件支持基本面 | 实时验证 | `news.sentiment_summary` |

**核心铁律:** 量在价先,**资金比形态更重要**。低位+地量只是必要条件,不是充分条件。资金流转正才是确认。

**必含维度(其他可缺):** 位置 + 资金流 + 量比 + 金叉。这 4 项缺任何一项都不算理想买点。

### 10.2 三种典型买入形态

#### 形态 A:成长股黄金坑(最理想,长期持有)
- **公司画像:** ROIC > 12% 且稳定(cv<0.3) + 利润持续正增长 + FCF/NI ≥ 0.5 + 巴菲特三步全过
- **行业:** 医药、消费电子、半导体设计、软件 SaaS、新能源细分龙头
- **买点:** 120日分位 < 30% + 量增价涨 + 低位单峰 + 资金流入 + 五步 5/5
- **持有:** 长期,时间是优势
- **止损:** 跌破筹码支撑或 ROIC 拐点

#### 形态 B:周期股低谷(波段布局)
- **公司画像:** 行业亏损或 PE 极高(利润低谷) + 资产负债表稳
- **行业:** 化工(细分)、有色、煤炭、航运、猪肉、面板
- **买点:** 120日分位 < 20% + **底背离**(价新低 + 量不新低)+ 资金流转正 + 行业产能出清信号
- **持有:** 波段,高峰离场(PE 极低 + 利润暴增 = 卖出)
- **止损:** 跌破低谷平台

#### 形态 C:高控盘主升浪(趋势跟踪,轻仓)
- **公司画像:** 成长股 + 强叙事(AI/机器人/国产替代)+ 底仓不动
- **行业:** AI 算力、人形机器人、半导体设备、创新药
- **买点:** 120日分位 30-70%(中位,非高位)+ 量缩价涨+均线陡峭 + 底仓不动(涨幅 30%+ 保留率 > 80%)
- **持有:** 拿稳,跌破 5 日线或底仓松动减仓
- **逃顶:** 高位(> 80%)+ 历史天量 ≥2 次 + 换手 >15% = 立即逃顶

### 10.3 漏斗筛选流程(4 步)

**不要一只一只看,要批量筛:**

1. **质地筛(50-100 只):** ROIC > 12% 且稳定 + 利润正增长 + FCF 良好 + 巴菲特三步过 2 步以上
   - 半导体行业例外:跳过质地筛(短期信号优先,见矩阵 7)
2. **位置筛(10-20 只):** 120日分位 < 30%(形态 A/C)或 < 20%(形态 B)
3. **资金流筛(5-10 只):** 5日累计净流入 + ma5 > ma20 金叉 + 连续 ≥2 日流入
4. **五步筛(1-3 只):** 量比 1.5-3 + 金叉 + 站稳 3 日 全过

**最终买点:** 10 维度同时满足 ≥ 7 项,且必含 4 项(位置 + 资金流 + 量比 + 金叉)。

### 10.4 选股实操步骤

1. **问方向:** 用户要成长股(A)/周期股(B)/主升浪(C)?有无行业偏好?
2. **建池:** 按方向选行业龙头池(10-30 只)
3. **批量筛:** 按漏斗 4 步逐步过滤
4. **深度验证:** 对筛出的 3-5 只候选,逐只跑 `analyze.py --days 120`
5. **打分排名:** 按 10 维度打分,输出排名表
6. **推荐:** 1-2 只最符合的,说明理由 + 缺哪些维度(诚实告知不完美处)
7. **风险提示:** 即使推荐,也说明可能的风险 + 止损位

### 10.5 选股 vs 单股票分析的区别

| 维度 | 单股票分析 | 选股 |
|------|-----------|------|
| 触发 | 用户给代码/名称 | 用户要推荐/问方向 |
| 输入 | 1 只 | 池子(10-30 只) |
| 输出 | 9 段深度报告 | 排名表 + 1-2 只推荐 |
| 重点 | 全面诊断 | 漏斗筛选 + 维度打分 |
| 脚本 | analyze.py 1 次 | analyze.py N 次 + 批量筛选 |

### 10.6 选股的诚实原则

- **大部分时候答案是"现在没有符合的":** 不要为了推荐而推荐
- **理想买点 10 维度同时满足很少见:** 一年可能只有 2-3 次
- **不完美时诚实告知:** 推荐时说明缺哪些维度,让用户自己决定
- **等机会,不是找机会:** 现在没有就等,不要降低标准
- **不推荐的三种情况:** ① 全市场无符合 ② 用户指定行业但该行业无符合 ③ 候选都缺必含维度(位置/资金流/量比/金叉)

### 10.7 行业分层模式(成长/周期/价值三模式动态阈值)

**问题:** 10.1 的"位置<30%"是"低谷布局"逻辑,适合周期/价值股,但对 AI/半导体等高景气赛道过于保守--这些股票可能正处于主升浪,120日分位长期 50-80% 是常态,用 30% 一刀切会过滤掉所有主升浪标的。

**解决:** 按行业打模式标签,每个模式用不同位置阈值和核心信号。从 `indicators.sector_mode` 读取。

#### 10.7.1 三种模式定义

| 模式 | 适用行业 | 位置阈值 | 核心信号(step4) | 逻辑 |
|------|----------|----------|-----------------|------|
| **growth(成长)** | AI服务器/AI芯片/EDA/半导体/存储/模拟/光铜缆/PCB/散热/通信/新能源车/光伏/军工/创新药 | <60% | 量比+金叉(主升浪启动) | 高景气赛道,高位可继续涨,放宽位置 |
| **cyclical(周期)** | 煤炭/钢铁/化工/有色/黄金/猪肉/航运/证券 | <30% | 量比+金叉+突破(低谷反转) | 周期股必须低位+突破确认反转 |
| **value(价值)** | 银行/白酒/家电/食品饮料/红利/保险 | <40% | 量比+金叉(底部启动) | 防御股,低位启动即可 |

#### 10.7.2 资金流降级处理

当资金流数据源降级(THS,无5日累计)时,step3用**主力意图**替代:
- `chip_cross_validation.main_force_intent` 含"吸筹" -> step3 通过
- 含"派发" -> step3 失败 + 触发高位减仓信号

#### 10.7.3 成长模式特殊信号

成长模式额外输出三个信号(从 `sector_mode` 读取):

| 信号 | 条件 | 含义 | 操作 |
|------|------|------|------|
| 🔥 **主升浪信号** | 位置>50% + 量比 + 金叉 + 吸筹 | 高位主升浪启动 | 轻仓跟随,止损5日线 |
| 📍 **底部启动信号** | 位置<30% + 量比 + 金叉 + 吸筹 | 成长股黄金坑 | 逢低布局,半仓以上 |
| ⚠️ **高位减仓信号** | 位置>70% + 派发 | 主力派发 | 立即减仓 |

#### 10.7.4 漏斗4步(按模式动态)

| 步骤 | growth | cyclical | value |
|------|--------|----------|-------|
| 1.质地 | sector已分类 | sector已分类 | sector已分类 |
| 2.位置 | <60% | <30% | <40% |
| 3.资金流 | 资金流pass OR 主力吸筹 | 资金流pass OR 主力吸筹 | 资金流pass OR 主力吸筹 |
| 4.核心信号 | 量比+金叉 | 量比+金叉+突破 | 量比+金叉 |

**4/4全过 = 买点。** growth模式额外区分主升浪(高位)和底部启动(低位)两种买法。

#### 10.7.5 应用场景

- **单股票分析:** `analyze.py` 自动输出 `indicators.sector_mode`,报告第八段综合建议引用模式判断
- **选股漏斗:** 筛选脚本传 `sector` 参数,用 `SECTOR_MODE` 映射模式,按模式动态筛
- **行业判断:** 从 `fundamental.industry` 推断 sector(如"食品饮料(白酒)"->白酒->value),兜底用股票名称

#### 10.7.6 典型案例

| 股票 | 旧漏斗(位置<30%) | 新漏斗(模式动态) | 改进 |
|------|-------------------|-------------------|------|
| 寒武纪(AI芯片,120日47.8%) | 1/4(位置卡掉) | 2/4(growth<60%过) | ⬆️ 不再误判主升浪 |
| 比亚迪(新能源车,120日40.1%) | 1/4 | 2/4(growth<60%过) | ⬆️ |
| 五粮液(白酒,120日7.0%) | 3/4(差资金流) | 3/4(value<40%过) | = 但模式更准确 |
| 牧原股份(猪肉,120日31.7%) | 1/4(位置卡掉) | 1/4(cyclical<30%仍卡) | = 周期股维持严格 |

## 矩阵 11:散户五大反指(11 年回测验证)

**来源:** A 股 5000+ 股票 × 11 年历史数据回测,5 个散户最常用指标全部是稳定反指。这些指标不仅预测不了上涨,而且对预测下跌特别管用。看反指的目的不是做空(A 股难做空),而是**避雷,避免被收割**。

### 11.1 五大反指清单

| # | 反指 | 散户逻辑 | 回测真相 | 现有指标覆盖 |
|---|------|----------|----------|--------------|
| 1 | 强者恒强买龙头 | 追 20 日涨幅最大/涨停最多的龙头 | "最强组"是唯一 20 日后收益为负的组(涨多了要还债) | 部分覆盖:位置 >70% + 短期筹码全部获利盘 |
| 2 | 天量之后有天价 | 换手越充分后续越高 | 个股天量是顶部反指,不是天价 | ✅ 矩阵 1.5 已覆盖:换手 >15% 套现警戒 + 120 日 ≥2 次天量 = 阶段顶部 |
| 3 | 跟紧主力净流入 | 看特大单净流入跟主力 | 11 年回测每年稳定反指,能让你看到的都是想让你看到的 | ⚠️ 矩阵 8 已警示:不单看资金流,用筹码×资金流交叉验证 |
| 4 | 融资推动杠杆牛 | 融资余额快速上涨 = 聪明钱上杠杆 | 杠杆 ≠ 聪明,反指 | ❌ 未覆盖,需手动核查同花顺/东财融资余额 |
| 5 | 大富大贵龙虎榜 | 上榜 = 被资金青睐,跟游资喝汤 | 龙虎榜常客是接盘信号,不是喝汤 | ❌ 未覆盖,需手动核查龙虎榜频率 |

### 11.2 综合韭菜指数

把以上 5 个反指合成 1 个综合指数,最像散户会追的 10% 股票,未来 20 天平均跌 **1.31%**,十年如一日稳定。

### 11.3 与现有资金流逻辑的关系(重要)

**矩阵 8 的"主力意图" ≠ 单纯跟主力净流入。**

- ❌ 散户反指:看行情软件"特大单净流入"就追(11 年回测每年反指)
- ✅ 我们的逻辑:`chip_cross_validation.main_force_intent` = 筹码峰迁移 + 集中度变化 + 资金流方向 交叉验证
- 警告:即使交叉验证,资金流数据本身来自同花顺/东财公开源,**"能让你看到的都是想让你看到的"**。当资金流信号"过于明显"时(如超大单净流入 + 正面新闻 + 龙虎榜),反而要警惕诱多出货

### 11.4 反指应用规则(在报告"七、量价背离 + 主力骗局"和"九、综合建议"中引用)

| 反指 | 触发条件 | 警示等级 |
|------|----------|----------|
| 🔴 强者恒强 | 120 日分位 >70% + 5/10 日短期筹码全部在现价下方(获利盘多) + 量缩价涨 | 警惕 20 日内回调 |
| 🔴 天量见顶 | 换手率 >15% + 历史天量 ≥2 次(矩阵 1.5 已覆盖) | 阶段顶部信号 |
| 🟠 资金流反指 | `capital_flow.signals.main_force_action="吸筹"` 但股价高位(>70%) + 主力净流入"过于明显" + 正面新闻配合 | 警惕诱多出货 |
| 🟠 融资余额 | 手动核查:融资余额月环比 >+20% | 警惕杠杆牛反指 |
| 🟠 龙虎榜 | 手动核查:月内上榜 ≥3 次 | 警惕接盘风险 |

### 11.5 反指规避原则

1. **不追"20 日最强组"**(涨幅榜前 10% + 涨停数最多) - 涨多了要还债
2. **不信"主力净流入"作为单一信号** - 11 年回测每年反指
3. **不参与"龙虎榜常客"的接盘行情** - 上榜 ≠ 被青睐,是接盘
4. **警惕"融资余额快速上涨"** - 杠杆 ≠ 聪明
5. **天量即顶部**(矩阵 1.5 已覆盖) - 不存"天量后还有天价"的幻想

**核心心法:** 市场最有价值的机会,往往不在最热闹的地方,而在那些已经出现基本面支撑、估值仍有余地、交易尚未拥挤的角落。最热闹的地方 = 韭菜指数最高的地方 = 反指信号最强的地方。

## 矩阵 12:概念/板块轮动(扩散指标 + RRG)

**触发:** 用户问"哪个板块/概念有机会"、"轮动分析"、"主线判断"、"现在什么板块强"

**问题:** 散户做概念轮动易犯"只追涨幅"的错 - 等察觉时市场已完成第一轮定价,买在分歧阶段。

**核心方法:** 用扩散指标 + RRG 判断主线是否健康,避免追高。

### 12.1 扩散指标(真实赚钱效应)

**定义:** 概念内上涨个股市值占比 = Σ(上涨股市值) / Σ(概念内总市值)

| 扩散指标 | 含义 | 选股策略 |
|----------|------|----------|
| >70% | 普涨,健康上涨 | 选补涨股(涨幅后段 + 位置低) |
| 50-70% | 分化,主线明确 | 选龙头(涨幅前3 + 量能配合) |
| <50% | 弱势,赚钱效应差 | 警惕,不参与 |
| 高位下降 | 资金开始撤离 | 减仓信号 |

**与"只看涨幅"的区别:** 涨幅看指数,但指数可能被权重股拉抬;扩散指标看概念内"多少股票真的在涨",反映真实赚钱效应。

### 12.2 RRG 相对强度轮动图(四区)

| 区域 | 特征 | 含义 | 操作 |
|------|------|------|------|
| 领先 | 相对强度高 + 持续上行 | 主线龙头 | 跟随(扩散高时) |
| 改善 | 相对强度低 + 上行 | 回暖轮动 | 关注布局 |
| 转弱 | 相对强度高 + 下行 | 衰退 | 减仓 |
| 落后 | 相对强度低 + 下行 | 无资金 | 回避 |

**坐标:** X 轴 = 相对强度(vs 全市场),Y 轴 = 相对强度动量(变化率)

### 12.3 策略步骤

1. **初筛:** 扩散指标 >50% 的概念
2. **RRG 过滤:** 保留领先/改善区,排除转弱/落后
3. **选股:**
   - 扩散 50-70%(分化)-> 选龙头(涨幅前3 + 量能配合)
   - 扩散 >70%(普涨)-> 选补涨(涨幅后段 + 位置低)
4. **市场环境:** 大盘弱势时降仓位,强势时正常(参考矩阵 13)

### 12.4 与现有技能关系

- **矩阵 10 选股:** 可加一步前置筛选 - 用扩散+RRG 锁定板块,再从板块内选股
- **矩阵 11 反指:** "强者恒强"在板块级别的体现 - 已完成第一次定价的板块不追
- **数据获取:** 同花顺/东财概念板块成分股 + 实时涨跌,需手动核查(脚本未提供板块级数据)

### 12.5 核心原则

- **看扩散而非涨幅:** 涨幅看指数,扩散看赚钱效应
- **看 RRG 而非单日:** RRG 看趋势,单日看噪音
- **不追已完成第一次定价的板块:** 扩散已高 + RRG 转弱 = 接盘信号

## 矩阵 13:市场择时(五维度投票,2-4 周短中期)

**触发:** 用户问"现在仓位多少"、"大盘怎么看"、"加仓还是减仓"、"市场环境如何"

**问题:** 个股分析再好,逆市场趋势也难赚。需要市场级择时判断。

**核心方法:** 五维度投票,看多 100% / 中性 50% / 看空 0%。

### 13.1 五维度清单

| 维度 | 指标 | 看多 | 看空 |
|------|------|------|------|
| 估值(赔率) | ERP = 1/PE - 十年国债利率 | 高(股票便宜) | 低(股票贵) |
| 资金(杠杆进攻度) | 全市场融资买入额布林带 | 突破上轨 | 跌破下轨 |
| 技术 | 指数布林带 + 市场广度(上涨成交额占比-下跌占比) | 上轨突破 + 广度高 | 下轨突破 + 广度低 |
| 情绪(核心!) | 五因子交叉(见 13.2) | 至少3因子看多 | 至少3因子看空 |
| 基本面(反向底部) | CPI / PMI / EPU | 3项至少2项走弱(触发高赔率底部) | 3项至少2项走强 |

### 13.2 情绪维度五因子(核心,需交叉验证)

| 因子 | 含义 | 看多信号 | 看空信号 |
|------|------|----------|----------|
| 期权 PCR | 认沽/认购比 | PCR高(认沽多=恐慌机会) | PCR低(过度乐观) |
| 隐含波动率 IV | 期权隐含波动率 | 飙升(恐慌) | 低位(冷漠) |
| 期货会员持仓 | 机构多空方向 | 席位净多 | 席位净空 |
| 基差 | 期货-现货币差 | 低(贴水) | 高(升水=过度乐观) |
| 主力资金 | 大单净流入 | 净流入 | 净流出 |

**关键:** 单因子噪音大,**至少 3 个因子同向**才算有效信号。
...
- 5 维度看多 3 -> 仓位 50%(中性)
- 5 维度看多 ≤2 -> 仓位 0%(看空)
- 情绪维度五因子交叉为反向底部信号时,即使其他维度中性,也可左侧布局

### 13.4 与现有技能关系

- **个股"九、综合建议"的"仓位":** 必须参考市场择时,不逆市场趋势加仓
- **矩阵 11 反指之"融资余额快速上涨":** 与本矩阵"融资买入额布林带突破上轨=看多"看似矛盾。区别:布林带突破是短线进攻信号(2-4周),反指是中线顶部信号(月度级别)。短线做多但设好止损
- **数据获取:** ✅ 已代码化(IV/期货持仓/EPU 降级)。运行 `python3 market_timing.py` 输出五维度投票 + 仓位建议。模块:`scripts/market_timing.py`

### 13.5 降级说明

代码化版本相对原设计的降级:
- **IV(隐含波动率):** akshare 无直接接口,降级省略(用 PCR 反推)
- **期货会员持仓:** `futures_main_sina` 仅主力 K 线,降级用基差(期货 vs 沪深300 指数)替代
- **EPU(政策不确定性):** 无 akshare 接口,基本面维度仅用 CPI+PMI 两项
- **市场广度:** 新浪 `stock_zh_a_spot` 有涨跌幅但无成交额,用"上涨家数占比"替代"上涨成交额占比"

### 13.6 核心原则

- **不逆市场趋势:** 5 维度看空时,即使个股信号好也降仓位
- **情绪是核心但需交叉:** 单因子不可信,至少 2 因子同向(降级版)
- **基本面反向底部:** CPI+PMI 双弱 = 高赔率底部信号(反向思维)

## 矩阵 14:日内走势诊断(过程比结果重要)

**触发:** 用户问"今天 XX 怎么样"、"这根 K 线强弱"、"尾盘拉升算不算"、"今天放量是真是假"

**问题:** 仅看单日涨幅不能判断股票真实强弱,要看股价运行全过程。同样涨 3%,老黄牛型和尾盘突击型完全不同。

### 14.1 四大维度

| 维度 | 公式 | 强 | 弱 |
|------|------|----|----|
| 价格路径效率 | 实际涨幅 / 全天总波动幅度(最高-最低) | >0.7(资金共识强) | <0.3(分歧大) |
| 收盘位置 | (收盘-最低) / (最高-最低) | >0.8(买方强势) | <0.5(套牢早盘资金) |
| 量价转化 | 涨幅 / 换手率 | 高(资金效率好) | 低(成交大涨幅小=分歧/出货) |
| 尾盘涨幅占比 | 尾盘1小时涨幅 / 全天涨幅 | <30%(持续上涨) | >50%(尾盘突击,持续性存疑) |

### 14.2 四类走势

| 类型 | 特征 | 含义 | 操作 |
|------|------|------|------|
| 🐂 老黄牛型 | 路径平滑 + 回撤小 + 收盘高位 | 资金一致,买盘持续 | ✅ 跟随 |
| 🎭 戏精型 | 振幅大 + 多次反转 | 多空博弈,分歧大 | ⚠️ 观望 |
| 📉 装忙型 | 成交活跃 + 涨幅小 | 量价不匹配,可能出货 | ❌ 警惕 |
| ⚡ 尾盘突击型 | 尾盘短时拉升 + 全天涨幅集中 | 持续性存疑,可能是诱多 | ❌ 不追 |
...
- **过程比结果重要:** 同样涨 3%,老黄牛型可追,尾盘突击型不可追
- **量价转化看效率:** 同等换手涨幅高 = 资金效率好,成交大涨幅小 = 分歧/出货
- **尾盘突击警惕:** 尾盘短时拉升稳定性差,次日易回调
...
### 反指规避核查(矩阵 11)
- [ ] 是否处于"20 日最强组"?(120日分位>70% + 短期筹码全部获利 + 量缩价涨 -> 警惕20日内回调)
- [ ] 天量见顶?(换手>15% + 历史天量≥2次 -> 阶段顶部)
- [ ] 资金流"过于明显"?(吸筹信号 + 高位 + 主力净流入异常明显 + 正面新闻配合 -> 警惕诱多出货)
- [ ] 融资余额快速上涨?(手动核查同花顺,月环比>+20% -> 杠杆牛反指)
- [ ] 龙虎榜常客?(手动核查,月内≥3次上榜 -> 接盘风险)

### 多层级环境核查(矩阵 12/13/14,触发式输出)
- [ ] **板块轮动(矩阵 12):** 所属板块扩散指标?(>50% 主线健康,<50% 警惕) + RRG 区域?(领先/改善可参与,转弱/落后回避)
- [ ] **市场择时(矩阵 13):** 五维度投票?(≥4 看多=100% 仓位,3 看多=50%,≤2 看多=0%)。情绪维度五因子是否同向?
- [ ] **日内走势(矩阵 14,用户问"今天"时输出):** 路径效率?(>0.7 强) + 收盘位置?(>0.8 买方强) + 尾盘涨幅占比?(>50% 警惕) -> 老黄牛型/戏精型/装忙型/尾盘突击型?
```
...
- **🔥 主升浪跟随(growth模式):** sector_mode.breakout_signal=True(高位>50% + 量比 + 金叉 + 吸筹)-> 轻仓跟随趋势,止损5日线,不死扛
- **📍 成长股黄金坑(growth模式):** sector_mode.bottom_signal=True(低位<30% + 量比 + 金叉 + 吸筹)-> 逢低布局,半仓以上长期持有
- **⚠️ 高位减仓(growth模式):** sector_mode.warn_signal=True(高位>70% + 派发)-> 立即减仓,不等的
- **模式决定策略:** growth模式顺势(高位可买),cyclical模式逆势(只买低位+突破),value模式买底部启动
- **🔴 反指规避(矩阵 11):** 不追"20 日最强组"(涨幅榜前 10%,涨多了要还债)/ 不信"主力净流入"单一信号(11 年回测每年反指)/ 不参与"龙虎榜常客"接盘 / 警惕"融资余额快速上涨"杠杆牛反指 / 天量即顶部(矩阵 1.5 已覆盖)
- **🟠 资金流"过于明显"警示:** 当 `capital_flow.signals.main_force_action="吸筹"` + 高位(>70%) + 主力净流入"意外地非常明显" + 正面新闻配合 = 警惕主力诱多出货(矩阵 11.3)。能让你看到的都是想让你看到的
- **🟠 强者恒强反指:** 120日分位 >70% + 5/10 日短期筹码全部在现价下方(获利盘多) + 量缩价涨 = "20 日最强组"特征,警惕 20 日内回调(11 年回测唯一负收益组)
- **🔵 板块轮动(矩阵 12):** 选股前先看板块扩散指标 + RRG。扩散 >50% + RRG 领先/改善区 = 主线健康;扩散 <50% 或 RRG 转弱/落后 = 不参与。不追已完成第一次定价的板块
- **🟣 市场择时(矩阵 13):** 个股"仓位"必须参考五维度投票。5 维度看空(≤2 看多)时,即使个股信号好也降仓位至 0-30%。情绪维度五因子至少 3 同向才算有效
- **🟤 日内走势(矩阵 14):** 看过程不看结果。老黄牛型(路径平滑+收盘高位)✅ 跟随;尾盘突击型(尾盘涨幅占比 >50%)❌ 不追;装忙型(成交大涨幅小)❌ 警惕出货。单日涨幅相同,强弱天差地别

## 选股操作原则(矩阵 10 应用)

- **选股触发:** 用户问"帮我选股/推荐/有什么能买的/哪个行业值得看"等方向性问题,走矩阵 10 漏斗,不要逐只分析
- **必含 4 维度:** 位置(< 30%)+ 资金流(流入+金叉)+ 量比(1.5-3)+ 均量线金叉 - 缺任何一项都不算理想买点,只能"关注"不能"买入"
- **资金比形态重要:** 低位+地量是必要条件,资金流转正才是确认。形态像底但资金流出 = 等待,不抄底(如正丹股份案例)
- **不为了推荐而推荐:** 大部分时候答案是"现在没有符合的",诚实告知用户等机会,不要降低标准凑数
- **方向先行:** 选股前先问用户要成长股(形态 A)/周期股(形态 B)/主升浪(形态 C),不同形态筛选条件不同
- **半导体选股例外:** 半导体行业跳过质地筛(矩阵 7 短期信号优先),直接从位置+资金流+筹码筛选
- **候选深度验证:** 漏斗筛出的 3-5 只候选必须逐只跑 `analyze.py --days 120` 完整分析,不能只看少量指标就推荐
- **诚实排名:** 推荐时说明每只候选的 10 维度满足情况 + 缺哪些维度,让用户自己决策
